...
class CrossoverAndMutation(object):
    def __init__(self, prob_crossover, prob_mutation, _log, individuals, _params=None):
        ...
        # storing other parameters if needed, such as the index for SXB and polynomial mutation
        self.params = _params
        self.log = _log
        self.offspring = []
    ...
    def process(self):
        ...
        Utils.save_population_after_crossover(
            self.individuals_to_string(), self.params['gen_no'])
...
class Crossover(object):
    ...
    def _calculate_pool_numbers(self, parent1, parent2):
        ...
        pos1, pos2 = int(np.floor(np.random.random()*len1)
                         ), int(np.floor(np.random.random()*len2))
    ...
    def do_crossover(self):
        ...
        for _ in range(len(self.individuals)//2):
            ...
            parent1, parent2 = copy.deepcopy(
                self.individuals[ind1]), copy.deepcopy(self.individuals[ind2])
            p_ = random.random()
            if p_ < self.prob:
                ...
                while first_begin_is_pool is True or second_begin_is_pool is True:
                    pos1, pos2, pool_len1, pool_len2 = self._calculate_pool_numbers(
                        parent1, parent2)
                    try_count = 1
                    while pool_len1 > self.pool_limit or pool_len2 > self.pool_limit:
                        ...
                        try_count += 1
                        self.log.warn(
                            'The %d-th try to find the position for do crossover' % (try_count))
                    self.log.info('Position %d for %s, positions %d for %s' % (
                    ...
                    if first_begin_is_pool is True:
                        self.log.warn(
                            'Crossovered individual#1 starts with a pooling layer, redo...')
                    if second_begin_is_pool is True:
                        self.log.warn(
                            'Crossovered individual#2 starts with a pooling layer, redo...')
...
class Mutation(object):
        ...
        for indi in self.individuals:
            ...
            if p_ < self.prob:
                ...
                if mutation_type == 0:
                    ...
                    raise TypeError(
                        'Error mutation type :%d, validate range:0-4' % (mutation_type))
            else:
    ...
    def do_add_unit_mutation(self, indi):
        ...
        self.log.info('A %s unit would be added due to the probability of %.2f' % (
            'CONV' if type_ == 1 else 'POOLING', u_))
        if type_ == 2:
            ...
            if num_exist_pool_units > StatusUpdateTool.get_pool_limit()[1]-1:
                type_ = 1
                self.log.info('The added unit is changed to CONV because the existing number of POOLING exceeds %d, limit size:%d' % (
        ...
        if type_ == 2:
            ...
            add_unit = indi.init_a_conv(
                mutation_position+1, _in_channel=_in_channel, _out_channel=None)
    ...
    def do_remove_unit_mutation(self, indi):
        ...
        if len(indi.units) > 1:
            # the first unit would not be removed
            mutation_position = int(
                np.floor(np.random.random()*(len(indi.units)-1))) + 1
            self.log.info('Mutation position occurs at %d' %
                          (mutation_position))
            ...
            self.log.warn(
                'REMOVE mutation can not be performed due to it has only one unit')
    ...
    def do_modify_conv_mutation(self, indi):
        ...
        if len(conv_index_list) == 0:
            self.log.warn(
                'No CONV unit exist in current individual, no mutation occurs')
        else:
            selected_index = int(
                np.floor(np.random.rand()*len(conv_index_list)))
            self.log.info('Mutation position %d' %
                          (conv_index_list[selected_index]))
            ...
            if indi.units[conv_index_list[selected_index]].in_channel != channel_list[index_]:
                ...
                if selected_index > 0:
                    self.log.info('Unit at %d changes its input channel from %d to %d' % (
                        conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].in_channel, channel_list[index_]))
                    indi.units[conv_index_list[selected_index]
                               ].in_channel = channel_list[index_]
                    self.log.info('Due to above, the unit at %d should change its output channel from %d to %d' % (
                        conv_index_list[selected_index-1], indi.units[conv_index_list[selected_index-1]].out_channel, channel_list[index_]))
                    indi.units[conv_index_list[selected_index-1]
                               ].out_channel = channel_list[index_]
                else:
                    self.log.warn(
                        'Mutation position is 0, the input channel should not be changed')
            else:
                self.log.info('Unit at %d changes its input channel from %d to %d' % (
                    conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].in_channel, channel_list[index_]))
            ...
            if indi.units[conv_index_list[selected_index]].out_channel != channel_list[index_]:
                ...
                self.log.info('Unit at %d changes its out channel from %d to %d' % (
                    conv_index_list[selected_index], indi.units[conv_index_list[selected_index]].out_channel, channel_list[index_]))
                indi.units[conv_index_list[selected_index]
                           ].out_channel = channel_list[index_]
                if selected_index < len(conv_index_list)-1:
                    self.log.info('Due to above, the unit at %d should change its input channel from %d to %d' % (
                        conv_index_list[selected_index+1], indi.units[conv_index_list[selected_index+1]].in_channel, channel_list[index_]))
                    indi.units[conv_index_list[selected_index+1]
                               ].in_channel = channel_list[index_]
                else:
                    self.log.info(
