...
# Convolutional Unit when r < 0.5
class ConvUnit(Unit):
    def __init__(self, number, in_channel, out_channel):
...
# Pooling layer when r >= 0.5
class PoolUnit(Unit):
    def __init__(self, number, max_or_avg):
        super().__init__(number)
        self.type = 2
        self.max_or_avg = max_or_avg     # maxpool for q < 0.5 and mean for q > 0.5
...
class Individual(object):
    def __init__(self, params, indi_no):
        self.acc = -1.0
        self.id = indi_no  # to record the id of the current number
        self.number_id = 0
    ...
    def initalize(self):
        ...
        # Initialize the layers based on there positions
    ...
    """
    Initialze a convolutional layer
    """
    def init_a_conv(self, _number, _in_channel, _out_channel):
        ...
        if _out_channel:
            ...
            out_channel = self.output_channel[np.random.randint(0, len(self.output_channel))]
        conv = ConvUnit(number, _in_channel, out_channel)
        return conv

    
    """
    Initialze a pooling layer
    """
    ...
    """ 
    For hash_key generation using SHA-224 and UTF-8
    """
    def uuid(self):
        _str = []
    ...
    """
    Gives info about the whole unit in a list called _str
    """
    def __str__(self):
        _str = []
        _str.append('indi:%s'%(self.id))
        _str.append('Acc:%s'%(self.acc))
        for unit in self.units:
            _sub_str = []
            if unit.type == 1:
                _sub_str.append('conv')
...
# Defining Population class that will store all the individuals
class Population(object):

    def __init__(self, params, gen_no):
        self.gen_no = gen_no
        self.number_id = 0     # to record how many individuals have been generated
        self.pop_size = params['pop_size']
        self.params = params
        self.individuals = []

    # Initializing individuals with population
    def initialize(self):
    ...
    # Gives info about the population in form of string
...
# Defining fake parameters to test above written code
def test_individual():
    params = {}
    params['min_conv'] = 30
    params['max_conv'] = 40
