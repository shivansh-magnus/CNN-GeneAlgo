...
class Individual(object):
    ...
    def initialize(self):
        ...
        for i in range(num_conv):
            # 1 denotes the convolution layer, and 2 denotes the pooling layer
        ...
        for i in all_positions:
            if i == 1:
                conv = self.init_a_conv(
                    _number=None, _in_channel=input_channel, _out_channel=None)
    ...
    def init_a_conv(self, _number, _in_channel, _out_channel):
        ...
        if _out_channel:
            ...
            out_channel = self.output_channles[np.random.randint(
                0, len(self.output_channles))]
