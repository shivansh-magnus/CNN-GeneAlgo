...
from genetic.fitness_eval import FitnessEvaluate
...
class EvolveCNN(object):
    ...
    def initialize_population(self):
        ...
        pops = Population(self.params, 0)   # Check this line for error
        pops.initialize()
        self.pops = pops
    ...
    def crossover_and_mutation(self):
        aft = CrossoverAndMutation(self.params['genetic_prob'][0], self.params['genetic_prob']
                                   [1], Log, self.pops.individuals, _params={'gen_no': self.pops.gen_no})
        offspring = aft.process()
    ...
    def environment_selection(self):
        ...
        selected = Selection()
        selected_index_list = selected.RouletteSelection(
        ...
        """Here, the population information should be updated, such as the gene no and then to individual id"""
    ...
    def do_work(self, max_gen):
        ...
        if StatusUpdateTool.is_evolution_running():
            Log.info('Initialize the existing population data')
